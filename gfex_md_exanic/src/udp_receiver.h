...
//     uint32_t    cpu_no;
//     size_t      receive_buffer_size = 8196;
// };


using UDPDataCallback = std::function<void(void *ctx, const char* data, size_t size,  const std::string& source_ip, uint16_t source_port)>;
...
public:
    ...
    MulticastReceiver(std::string multicast_ip, uint16_t multicast_port, std::string interface_name, int32_t cpu_no = -1, size_t receive_buffer_size = 8196)
    {
        ...
        if (inet_pton(AF_INET, m_multicast_ip.c_str(), &m_multicast_addr.sin_addr) <= 0) {
            ...
            std::clog <<__func__<<","<< __LINE__<< ",interface_name:" << m_interface_name.c_str() << ",multicast_ip:" << m_multicast_ip.c_str() << ",multicast_port:" << m_multicast_port << std::endl;
            return;
            // throw std::runtime_error("Invalid multicast IP address: " + m_multicast_ip);
        }
...
private:
    ...
    bool initializeSocket(){
        ...
        if (m_socket_fd < 0) {
            std::clog << "Failed to create socket: " << strerror(errno) << std::endl;
            return false;
        }
        ...
        if (setsockopt(m_socket_fd, SOL_SOCKET, SO_REUSEADDR, &reuse, sizeof(reuse)) < 0) {
            std::clog << "Failed to set SO_REUSEADDR: " << strerror(errno) << std::endl;
            return false;
        }
        ...
        if (setsockopt(m_socket_fd, SOL_SOCKET, SO_RCVBUF, &buffer_size, sizeof(buffer_size)) < 0) {
            std::clog << "Failed to set receive buffer size: " << strerror(errno) << std::endl;
        }
        ...
        if (bind(m_socket_fd, (struct sockaddr*)&local_addr, sizeof(local_addr)) < 0) {
            std::clog << "Failed to bind socket: " << strerror(errno) << std::endl;
            return false;
        }
        
        std::clog <<__func__<<","<< __LINE__<< ",m_socket_fd:" << m_socket_fd << ",buffer_size:" << buffer_size << std::endl;
        return true;
    }
    ...
    void receiveThread(){
        ...
        while (m_running) {
            ...
            //     // std::clog <<__func__<<","<< __LINE__<< ",size:"<<recv_len<<",source_ip:"<< source_ip_str<<",source_port:"<<source_port<<",errno:"<<errno<< std::endl;
            // }
            ...
            if (m_recv_callback) {
                m_recv_callback(m_ctx, buffer, recv_len, source_ip_str, source_port);
            }
        }
    }
    ...
    uint32_t getInterfaceIndex(const std::string& interface_name){
        ...
        if (sock < 0) {
            std::cerr << "Failed to create socket for interface index: "<<strerror(errno)<< std::endl;
            return -1;
        }
    ...
    bool joinMulticastGroup(){
        ...
        if (ifindex < 0) {
            std::clog <<__func__<<","<< __LINE__<< "Failed to get interface index for " << m_interface_name << std::endl;
            return false;
        }

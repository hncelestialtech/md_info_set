...
void *create() {
    printf("create GFEXExanicQuota[%s]\n");
    return new GFEXExanicQuota();
}
void destroy(void *p) {
    printf("destroy GFEXExanicQuota\n");
    delete (GFEXExanicQuota*)p;
}
...
inline bool GFEXExanicQuota::level1_belongTo(uint32_t ip, uint16_t port){
...
inline bool GFEXExanicQuota::level2_belongTo(uint32_t ip, uint16_t port){
...
GFEXExanicQuota::GFEXExanicQuota(): 
    m_fb_spi(nullptr),
...
void GFEXExanicQuota::register_spi(cffex::fb::api::fb_i_md_spi *spi){
    std::clog <<__func__<<","<< __LINE__<< std::endl;
    m_fb_spi = spi;
}

int GFEXExanicQuota::init(){
    std::clog <<__func__<<","<< __LINE__<< std::endl;
    LoadJsonCfg();
...
void GFEXExanicQuota::connect(){
    std::clog <<__func__<<","<< __LINE__<<",success"<< std::endl;

    m_dispatcher = new std::thread(&GFEXExanicQuota::ProcessMsg, this, m_config.cpu_id[0]);
    ...
    m_udp_receiver.reset(new MulticastReceiver(m_config.tcpconfig.ip, m_config.tcpconfig.port, m_config.tcpconfig.ifname, m_config.cpu_id[2], 2048)); //因为数据长度是135,所以buffer长度选择256
...
void GFEXExanicQuota::subscribe_inst(const std::string &instrument_id, uint8_t exchange_id){
...
inline void GFEXExanicQuota::LoadJsonCfg(){
    std::clog <<__func__<<","<< __LINE__<< std::endl;
...
void GFEXExanicQuota::ProcessMsg(int32_t cpu_id) {
    sleep(2);
    BindCPU(cpu_id);
    Slot slot;
    while (true) {
...
void GFEXExanicQuota::OnData_lv2(const char* databuffer, size_t size){
...
inline int32_t GFEXExanicQuota::IsValidQuota(QuotaInfo *cache, uint64_t extime, uint64_t vol, uint32_t level, double ap1, double bp1, uint32_t av1, uint32_t bv1){
...
void GFEXExanicQuota::UdpHandler(void *ctx, const char* data, size_t size,  const std::string& source_ip, uint16_t source_port){
    GFEXExanicQuota *pthis = (GFEXExanicQuota *)ctx;
    pthis->OnData_lv1(data, size);
}


void GFEXExanicQuota::ExanicHandler(void *ctx, const char* data, size_t size){
...
void GFEXExanicQuota::OnData_lv1(const char* data, size_t size){
...
void GFEXExanicQuota::DispatchMessage(const Slot& slot) {

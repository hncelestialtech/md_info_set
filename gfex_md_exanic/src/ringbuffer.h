...
struct DataQuote
{
    ...
    DataQuote():extime(0),last_price(FLOAT64_NAN),openinterest(0),volume(0),turnover(FLOAT64_NAN),
    ...
    DataQuote(uint64_t extime, char *inst, size_t inst_len, double last_price, int32_t openinterest,int32_t volume, double turnover, 
        ...
        strncpy(this->inst, inst, inst_len);
        this->inst[inst_len] = '\0';
        // std::clog <<__func__<<","<< __LINE__<<",inst:"<<this->inst<<",inst:"<<inst<<"inst_len:"<<inst_len<< std::endl;

...
public:
    ...
    __attribute__((always_inline)) void push_l1(const NanoGfexL1MdType& msgdata, uint64_t extime, uint64_t t0, uint64_t t1, uint32_t res) {
        ...
        while (true) {
            ...
            if (!is_full(current_read, current_write)) {
                ...
                memcpy(&slot.l1_data, &msgdata, sizeof(NanoGfexL1MdType));
    ...
    //推送L2数据到队列
    __attribute__((always_inline)) void push_l2(const NanoGfexL2MdType& msgdata, uint64_t extime, uint64_t t0, uint64_t t1, uint32_t res) {
        uint64_t seq = m_buffer.sequence.fetch_add(1, std::memory_order_relaxed);
        ...
        while (true) {
            ...
            if (!is_full(current_read, current_write)) {
                ...
                memcpy(&slot.l2_data, &msgdata, sizeof(NanoGfexL2MdType));

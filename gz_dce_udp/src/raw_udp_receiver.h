...
public:
    ...
    RawUdpReceiver(UdpConfig &config)
    {
        ...
        std::clog <<__func__<<","<< __LINE__<< ",m_socket_fd:" << m_socket_fd<< std::endl;
        ...
        std::clog <<__func__<<","<< __LINE__<< ",ifindex:" << ifindex<< std::endl;
...
private:
    ...
    void receiveThread(){
        ...
        std::clog <<__func__<<","<< __LINE__<< ",m_socket_fd:" << m_socket_fd<< ",m_running:" << m_running << std::endl;

        char buffer[RECEIVE_BUFFER_SIZE];
        while (m_running) {
            ssize_t recv_len = recv(m_socket_fd, buffer, RECEIVE_BUFFER_SIZE, 0);
            //std::clog <<__func__<<","<< __LINE__<< ",recv msg len:" << recv_len<< std::endl;
            if (recv_len <= 0) {
                if (!m_running) {
                    break;
                }
            ...
            //std::clog <<__func__<<","<< __LINE__<<",eth->h_proto:"<<ntohs(eth->h_proto)<<",ETH_P_IP:"<<ETH_P_IP<< std::endl;
			
			if (ntohs(eth->h_proto) != ETH_P_IP) continue;
			
			struct iphdr* ip = (struct iphdr*)(buffer + sizeof(struct ethhdr));
			
			// std::string ip_str = uint32_to_ip_safe(ip->daddr);			

            // std::clog <<__func__<<","<< __LINE__<<",ip->daddr:"<<ip->daddr<<",m_config.net_dst_ip:"<<m_config.net_dst_ip<<",ip_str:"<<ip_str.c_str()<< std::endl;

            if (ip->daddr != m_config.net_dst_ip) continue;
			
            // std::clog <<__func__<<","<< __LINE__<<",ip->protocol:"<<ip->protocol<<",IPPROTO_UDP:"<<IPPROTO_UDP<< std::endl;
            ...
            // uint16_t dstPort_local = ntohs(udp->dest);
			
			int offset = sizeof(struct ethhdr) + ip_header_len + sizeof(struct udphdr);
			
			int payload_len = recv_len - offset;
            // std::clog <<__func__<<","<< __LINE__<<",udp->dest:"<<udp->dest<<",m_config.net_dst_port:"<<m_config.net_dst_port<< std::endl;
			
            if (udp->dest != m_config.net_dst_port) continue;

            if (m_recv_callback) {
				// hexDumpToClog(buffer,recv_len);
                // std::clog <<__func__<<","<< __LINE__<< ",offset:" << offset<< ",recv msg len:" << recv_len<< ",payload_len:" << payload_len<< std::endl;
                m_recv_callback(m_ctx, buffer + offset, payload_len);
            }
        }
    }

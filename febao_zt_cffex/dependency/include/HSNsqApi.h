...
public:
    ...
    /// Description: 主推-现货盘后固定逐笔成交行情 (暂不支持)
    ...
    ///        SDK登录到sailfish_service之后，sailfish_service初始化成功，SDK才能收到该通知。
...
public:
    ...
    /// Description: 初始化连接
    ///              pszLicFile            通讯证书(未使用)
    ///              pszSafeLevel          安全级别(未使用)
    ///              pszPwd                通讯密码(未使用)
    ///              pszSslFile            SSL证书(未使用)
    ///              pszSslPwd             SSL密码(未使用)
    ...
    /// Description: 客户登录
    ///              pReqUserLogin          用户登录信息 AccountID、Password
    ///              nRequestID             请求ID; 目前无用，填0即可
    /// Return     ：int 0表示登录请求成功，否则表示失败，通过调用GetApiErrorMsg可以获取详细错误信息
    ...
    /// Description: 注册Fens网络地址 (空实现，无需调用)
    ...
    ///              nRequestID                                请求编号 
    /// Return     : int 0表示请求成功(订阅结果通过OnRspFutuDepthMarketDataSubscribe返回)；否则表示失败，通过调用GetApiErrorMsg可以获取详细错误信息
    /// Note       : 订阅成功后，数据通过OnRtnFutuDepthMarketData主推
    ...
    ///              nRequestID                             请求编号 
    /// Return     : int 0表示请求成功(退订结果通过OnRspFutuDepthMarketDataCancel返回)；否则表示失败，通过调用GetApiErrorMsg可以获取详细错误信息
    ...
    ///              nRequestID 					      		请求编号 
    /// Return     : int 0表示请求成功(订阅结果通过OnRspSecuDepthMarketDataSubscribe返回)；否则表示失败，通过调用GetApiErrorMsg可以获取详细错误信息
    /// Note       : 订阅成功后，数据通过OnRtnSecuDepthMarketData等主推
    ...
    ///              nRequestID                             请求编号    
    /// Return     : int 0表示请求成功(结果通过OnRspSecuDepthMarketDataCancel返回)；否则表示失败，通过调用GetApiErrorMsg可以获取详细错误信息                  
    ...
    ///              nRequestID                             请求编号
    /// Return     : int 0表示请求成功(订阅结果通过OnRspSecuTransactionSubscribe返回)；否则表示失败，通过调用GetApiErrorMsg可以获取详细错误信息
    /// Note       : 订阅成功后，逐笔行情通过OnRtnSecuTransactionTradeData、OnRtnSecuTransactionEntrustData等主推 
    ...
    ///              nCount                                 nCount必须填0，仅支持查询pReqQrySecuInstruments[0].ExchangeID全市场
    ///              nRequestID                             请求编号 
    ...
    ///              nRequestID                             请求编号 
    /// Return     : int 0表示请求成功(订阅结果通过OnRspOptDepthMarketDataSubscribe返回)，否则表示失败，通过调用GetApiErrorMsg可以获取详细错误信息
    /// Note       : 订阅成功后，数据通过OnRtnOptDepthMarketData主推
    ...
    ///              nRequestID                             请求编号
    /// Return     : int 0表示请求成功(退订结果通过OnRspOptDepthMarketDataCancel返回)，否则表示失败，通过调用GetApiErrorMsg可以获取详细错误信息
    ...
    ///              nCount 						      		订阅行情合约个数。nCount为0，表示订阅pReqSecuDepthMarketDataSubscribe[0].ExchangeID全市场
    ///              nRequestID 					      		请求编号 
    /// Return     : int 0表示请求成功(订阅结果通过OnRspSecuDepthMarketDataPlusSubscribe返回)；否则表示失败，通过调用GetApiErrorMsg可以获取详细错误信息
    /// Note       : 订阅成功后，数据通过OnRtnSecuDepthMarketDataPlus主推
    ...
    ///              nCount                                 取消订阅行情合约个数。nCount为0，表示取消订阅pReqSecuDepthMarketDataCancel[0].ExchangeID全市场
    ///              nRequestID                             请求编号     
    /// Return     : int 0表示请求成功(结果通过OnRspSecuDepthMarketDataPlusCancel返回)；否则表示失败，通过调用GetApiErrorMsg可以获取详细错误信息             
    ...
    ///              nRequestID                             请求编号 
    /// Return     : int 0表示请求成功(结果通过OnRspSecuTransactionData返回)；否则表示失败，通过调用GetApiErrorMsg可以获取详细错误信息             
    ...
    ////以下是扩展订阅接口，目前仅用于对接 HDF2.0 接收网关场景；非该场景使用时直接返回失败
    /// Description: ProductID订阅-行情请求
    /// Input      : pReqSubscribeField[]                       行情订阅请求结构体数组
    ///              nCount 						      		订阅行情Productid个数。
    ///              nRequestID 					      		请求编号 
    /// Return     : int  0表示请求完成(请求结果通过OnRspProIDSubscribe返回，推送数据通过OnRtnXxxxxData回调)
    virtual int ReqProIDSubscribe(CHSNsqReqProductIDField pReqSubscribeField[], int nCount, int nRequestID) = 0;

    /// Description: ProductID订阅取消-行情请求
    /// Input      : pReqCancelField[]                      行情取消订阅请求结构体数组 
    ///              nCount                                 取消订阅行情Productid个数
    ///              nRequestID                             请求编号        
    /// Return     : int 0表示请求完成(请求结果通过OnRspProIDSubscribeCancel返回)
    virtual int ReqProIDSubscribeCancel(CHSNsqReqProductIDField pReqCancelField[], int nCount, int nRequestID) = 0;

    /// Description: ProductID代码信息查询
    /// Input      : nProductID                             代码信息查询请求产品ID 
    ///              nRequestID                             请求编号 
    /// Return     : int 0表示请求成功(数据通过OnRspQrySecuInstruments返回); 否则表示失败，可通过调用GetApiErrorMsg可以获取详细错误信息
    virtual int ReqProIDQrySecuInstruments(HSQuoteProductID nProductID, int nRequestID) = 0;
...
extern "C"
{
    ...
    /// Return     : succ: 返回CHSNsqApi实例指针;  fail: 返回NULL (内存申请失败)

...
struct CHSNsqFutuDepthMarketDataField
{
    ...
    HSPrice                       MaAskPrice;
    /// 委买成交金额(上期,能源)
    HSBalance                     BidBalance;
    /// 委卖成交金额(上期,能源)
    HSBalance                     AskBalance;
    /// 总买量
    HSNum                         TotalBidVolume;
    /// 总卖量
    HSNum                         TotalAskVolume;
    /// 申买五档委托笔数(郑商)/推导量(大商)
    HSNum                         BidNumOrders[5];
    /// 申卖五档委托笔数(郑商)/推导量(大商)
    HSNum                         AskNumOrders[5];
};
...
struct CHSNsqSecuDepthMarketDataPlusField
{
    ...
    /// 触发快照的数据类型
    /// HS_TRANS_Trade：逐笔成交
    /// HS_TRANS_Entrust： 逐笔委托
    /// 其他：快照(快照优先)
    HSTransType                 TickType;
    /// 触发快照的逐笔消息序号
    HSSeqNo                     SeqNo;
    /// 触发快照的逐笔价格
    HSPrice                     TickPrice;
    /// 触发快照的逐笔量(委托量或成交量)
    HSIntVolume                 TickVolume;
} ;
...
struct CHSNsqReqSecuTransactionRebuildField
{
    ...
    // 双闭区间   /// 区间范围需限制在2000以内，单次请求-重建服务至多返回[BeginSeqNo,BeginSeqNo+1999]的数据
    ///起始序号
    HSSeqNo     	               BeginSeqNo;
    ///结束序号
    HSSeqNo     	               EndSeqNo;
...
struct CHSNsqSecuTransactionEntrustDataField {
    ...
    ///买卖方向
    /// SH: ('1':买单; '2':卖单); 如产品状态订单逐笔(OrdType=='S'), 该字段无值
    ...
    /// SH: 原始订单号(仅适用上交所)
    HSSeqNo                       OrdNo;
    /// SH: 逐笔成交与逐笔委托统一编号(仅适用上交所, 使用合并逐笔后该字段无意义) [弃用] 
    HSSeqNo                       BizIndex;
...
struct CHSNsqSecuInstrumentStaticInfoField {
    ...
    ///证券属性标记   
    HSSecuritiesSpecialMarker     SpecialMarker;
    /// 代码上市日期，格式为YYYYMMDD (上交所部分历史代码上市日期仅供参考)
    HSDate                        ListDate;

    /// 发行总量  SZ
    HSNum64                       OutstandingShare;
    /// 流通股数  SH, SZ
    HSNum64                       PublicFloatShareQuantity;
};
...
struct CHSNsqInstrumentsDataChangeNoticeField 
{
    ...
    HSDate                        TradeDate;
    ///产品编号(暂未使用，预留)
    HSQuoteProductID              ProductID;
};

///按产品编号订阅 数据结构
struct CHSNsqReqProductIDField
{
    /// 产品编号  ///取值见接口文档附录
    HSQuoteProductID             ProductID;
    /// 行情数据类型
    HSQuoteDataType              DataType;
};

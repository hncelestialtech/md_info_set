#ifndef FB_COMB_STOCK_STREAM_H
#define FB_COMB_STOCK_STREAM_H
...
class comb_stock_stream :
    public i_stream_with_id<comb_stock_stream, STRATEGY_STREAM_COMB_STOCK> {
public:
    ...
    public:
        ...
        virtual const i_stream_msg *get_msg(IN int64_t comb_id) const     = 0;
    ...
    class i_stream_notify_split_msg {
    public:
        virtual ~i_stream_notify_split_msg() {}
    ...
    typedef std::function<void(IN const i_stream_msg *msg,
        ...
        snap_callback;
    typedef std::function<void(IN const i_stream_notify_split_msg *msg)>
        notify_split_msg_callback;

public:
    comb_stock_stream(msg_callback callback, snap_callback s_callback);
    comb_stock_stream(msg_callback callback);
    comb_stock_stream();
    ~comb_stock_stream();
    ...
    virtual void on_notify_split_data(const void *data);
    void         set_msg_callback(msg_callback callback);
    void         set_snap_callback(snap_callback s_callback);
    void         set_notify_split_callback(
                notify_split_msg_callback notify_split_callback);

private:
    msg_callback              *callback_;
    snap_callback             *snap_callback_;
    notify_split_msg_callback *notify_split_callback_;

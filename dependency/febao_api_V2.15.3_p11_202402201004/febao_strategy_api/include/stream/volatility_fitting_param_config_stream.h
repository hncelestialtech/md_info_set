#ifndef FB_VOLATILITY_FITTING_PARAM_CONFIG_STREAM_H
#define FB_VOLATILITY_FITTING_PARAM_CONFIG_STREAM_H
...
class volatility_fitting_param_config_stream :
    public i_stream_with_id<volatility_fitting_param_config_stream,
...
public:
    ...
    public:
        ...
        virtual int8_t      get_fit_contract_range() const      = 0;
        virtual int8_t      get_fit_price_type() const          = 0;
        virtual double      get_upper_active_volatility() const = 0;
        virtual double      get_down_active_volatility() const  = 0;
        virtual int8_t      get_fit_range_type() const          = 0;
        virtual const char *get_fit_range_param() const         = 0;
    ...
    public:
        ...
        virtual const i_stream_msg *get_msg(IN const char *option_serial_id,
                                            IN int8_t
                                                fit_contract_type) const = 0;
        virtual const i_stream_msg *get_msg(IN uint64_t option_serial_index,
                                            IN int8_t
                                                fit_contract_type) const = 0;
...
public:
    volatility_fitting_param_config_stream(msg_callback  callback,
                                           snap_callback s_callback);
    volatility_fitting_param_config_stream(msg_callback callback);
    volatility_fitting_param_config_stream();
    ~volatility_fitting_param_config_stream();

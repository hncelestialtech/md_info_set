...
int wing_fit_impl::fit(const volatility_point *p_volatility_points,
...
bool wing_fit_impl::valid_price(double price) {
	return (price > -DBL_MAX && price < DBL_MAX);
}

bool wing_fit_impl::calculate_atm_volatility()
{
	wing_point point;
	put_size_ = 0;
	call_size_ = 0;
...
bool wing_fit_impl::calculate_cutoff() {
...
bool wing_fit_impl::calculate_slope_curvature(wing_point *a_point, unsigned int size, double &slope, double &curvature) {
	try
	{
		double totalX1 = 0.0;
		double totalX2 = 0.0;
...
bool wing_fit_impl::calculate_left_curvature(wing_point *a_point, unsigned int size, double &curvature) {
	try
	{
		double s1 = 0.0;
		double s2 = 0.0;
...
void wing_fit_impl::construct_wing_para(char fixed_model_param[MAX_FIXED_MODEL_PARAM_LENGTH], char variable_model_param[MAX_VARIABLE_MODEL_PARAM_LENGTH]) {

...
class fb_wing_static_algorithm : public i_volatility_algorithm {
public:
    fb_wing_static_algorithm() {}
    virtual ~fb_wing_static_algorithm() {}
...
void* create() {
    return new cffex::fb::plugin::fb_wing_static_algorithm();
}

void destroy(void* p) {
    delete (cffex::fb::plugin::fb_wing_static_algorithm *)p;
}

typedef void (*register_func)(void *handle, unsigned short id, const char *name);
void identity(void* handle,register_func f) {
    f(handle,0x0001,"wing_static_volatility_algorithm");
}

...
class fb_cubic_spline_mixed_algorithm : public i_volatility_algorithm {
public:
    fb_cubic_spline_mixed_algorithm() {}
    virtual ~fb_cubic_spline_mixed_algorithm() {}
...
void* create() {
    return new cffex::fb::plugin::fb_cubic_spline_mixed_algorithm();
}
void destroy(void* p) {
    delete (cffex::fb::plugin::fb_cubic_spline_mixed_algorithm *)p;
}

typedef void (*register_func)(void *handle, unsigned short id, const char *name);
void identity(void* handle,register_func f) {
    f(handle,0x0004,"cubic_spline_mixed_volatility_algorithm");
}

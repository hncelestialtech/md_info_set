...
class fb_crr_hpc_on_future_theoretical_algorithm : public i_theoretical_algorithm {
public:
	class input_params {
	public:
		input_params() {}
		input_params(unsigned short option_type, double F, double K, double T, double r, double sig, int steps)
			:option_type(option_type), F(F), K(K), T(T), r(r), sig(sig), steps(steps) {};
	public:
	...
	public:
		full_params() {}
		full_params(unsigned option_type, double F, double K, double T, double r, double sig, int steps, double theo_price, double delta, double gamma, double vega, double theta, double rho, double charm, double vanna, double vomma, double speed, double zomma)
			:input_(option_type, F, K, T, r, sig, steps), pricing_(theo_price, delta, gamma, vega), uncommon_greeks_(theta, rho, charm, vanna, vomma, speed, zomma) {};
	public:
		input_params input_;
	...
	fb_crr_hpc_on_future_theoretical_algorithm();
	virtual ~fb_crr_hpc_on_future_theoretical_algorithm();
...
private:
	int Exception(IN double F, IN double K, IN double T, IN double r, IN double sig, IN int steps);
...
private:
	int single_iter(IN unsigned short option_type, IN double F, IN double K, IN double T, IN double r, IN double sig, IN int steps, OUT double* theo_price, OUT double* delta, OUT double* gamma);
	int pricing_iter(IN unsigned short option_type, IN double F, IN double K, IN double T, IN double r, IN double sig, IN int steps, OUT double* theo_price, OUT double* delta, OUT double* gamma, OUT double* vega);
	int uncommon_greeks_iter(IN unsigned short option_type, IN double F, IN double K, IN double T, IN double r, IN double sig, IN int steps, OUT double* theta, OUT double* rho, OUT double* charm, OUT double* vanna, OUT double* vomma, OUT double* speed, OUT double* zomma);
	...
	double calc_theta(IN unsigned short option_type, IN double F, IN double K, IN double T, IN double r, IN double sig, IN int steps, IN double cur_price);
	double calc_vega(IN unsigned short option_type, IN double F, IN double K, IN double T, IN double r, IN double sig, IN int steps);
	double calc_rho(IN unsigned short option_type, IN double F, IN double K, IN double T, IN double r, IN double sig, IN int steps, IN double cur_price);

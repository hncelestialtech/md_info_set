...
#define MIN_STEPS 2
#define TIMER_INTERVAL_COMMON  3000
#define TIMER_INTERVAL_UNCOMMON  10000
...
class fb_crr_hpc_on_stock_theoretical_algorithm : public i_theoretical_algorithm {
public:
	class input_params {
	public:
		input_params() {}
		input_params(unsigned short option_type, double S0, double K, double T, double r, double sig, double q, int steps)
	...
	public:
		full_params() {}
		full_params(unsigned option_type, double S0, double K, double T, double r, double sig, double q, int steps, double theo_price, double delta, double gamma, double vega, double theta, double rho, double charm, double vanna, double vomma, double speed, double zomma)
			:input_(option_type, S0, K, T, r, sig, q, steps), pricing_(theo_price, delta, gamma, vega), uncommon_greeks_(theta, rho, charm, vanna, vomma, speed, zomma) {};
	public:
		input_params input_;
	...
	fb_crr_hpc_on_stock_theoretical_algorithm();
	virtual ~fb_crr_hpc_on_stock_theoretical_algorithm();
...
private:
	int Exception(IN double S0, IN double K, IN double T, IN double r, IN double sig, IN double q, IN int steps);
...
private:
	int single_iter(IN unsigned short option_type, IN double S0, IN double K, IN double T, IN double r, IN double sig, IN double q, IN int steps, OUT double* theo_price, OUT double* delta, OUT double* gamma);
	int pricing_iter(IN unsigned short option_type, IN double S0, IN double K, IN double T, IN double r, IN double sig, IN double q,IN int steps, OUT double* theo_price, OUT double* delta, OUT double* gamma, OUT double* vega);
	int uncommon_greeks_iter(IN unsigned short option_type, IN double S0, IN double K, IN double T, IN double r, IN double sig, IN double q, IN int steps, OUT double* theta, OUT double* rho, OUT double* charm, OUT double* vanna, OUT double* vomma, OUT double* speed, OUT double* zomma);
	int calc_high_order_greeks(IN unsigned short option_type, IN double S0, IN double K, IN double T, IN double r, IN double sig, IN double q, OUT double* charm, OUT double* vanna, OUT double* vomma, OUT double* speed, OUT double* zomma);
	double calc_theta(IN unsigned short option_type, IN double S0, IN double K, IN double T, IN double r, IN double sig, IN double q, IN int steps, IN double cur_price);
	double calc_vega(IN unsigned short option_type, IN double S0, IN double K, IN double T, IN double r, IN double sig, IN double q, IN int steps);
	double calc_rho(IN unsigned short option_type, IN double S0, IN double K, IN double T, IN double r, IN double sig, IN double q, IN int steps, IN double cur_price);

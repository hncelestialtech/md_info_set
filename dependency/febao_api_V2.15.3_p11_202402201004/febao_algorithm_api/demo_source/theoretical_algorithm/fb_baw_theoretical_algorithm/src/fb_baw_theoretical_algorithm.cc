...
#include "fb_baw_theoretical_algorithm.h"
...
extern "C"
{
	...
	void* create() {
		return new cffex::fb::plugin::fb_baw_theoretical_algorithm();
	}
	void destroy(void* p) {
		delete (cffex::fb::plugin::fb_baw_theoretical_algorithm*)p;
	}
	void identity(void* handle, register_func f) {
		f(handle, 5, "fb_baw_theoretical_algorithm");
	}
...
fb_baw_theoretical_algorithm::fb_baw_theoretical_algorithm() {
}

fb_baw_theoretical_algorithm::~fb_baw_theoretical_algorithm() {
}

void fb_baw_theoretical_algorithm::set_register_timer_function(register_timer_callback f) {
	f(TIMER_INTERVAL, std::bind(&fb_baw_theoretical_algorithm::on_timer_uncommon_greeks, this), true);
};

int fb_baw_theoretical_algorithm::calculate(IN unsigned short option_type, IN const theoretical_variable_param* variable_params, IN const theoretical_static_param* static_params, IN const theoretical_custom_param *custom_params, IN const theoretical_volatility_param *volatility_params, OUT theoretical_field* f){
	...
	if (Exception(F, K, T, r, sig) == -1){
		PLOG("fb_baw_theoretical_algorithm::%s,there are exceptional data:F[%lf] or K[%lf] or T[%lf] or r[%lf] or sig[%lf] or steps[%d] is invalid, failed to calculate\n", __FUNCTION__, F, K, T, r, sig);
		return -1;
	}
	T = std::max(T, T_MIN);
	//calculate theoretical price by baw for american futures option.
	...
	uncommon_greeks_baw(option_type, F, K, T, r, sig, &theta, &rho, &charm, &vanna, &vomma, &speed, &zomma);
	f->set_theta(theta);
	f->set_rho(rho);
	f->set_charm(charm);
	f->set_vanna(vanna);
...
int fb_baw_theoretical_algorithm::calculate(IN unsigned short option_type, IN const theoretical_variable_param* variable_params, IN const theoretical_static_param* static_params, IN const theoretical_custom_param *custom_params, IN const theoretical_volatility_param *volatility_params, OUT double* price){
	double F = variable_params->get_forward();
	double K = variable_params->get_strike();
	double T = variable_params->get_left_trading_years();
	double r = variable_params->get_rate();
...
int fb_baw_theoretical_algorithm::iv_derivable(IN unsigned short option_type, IN double option_price, IN const theoretical_variable_param* variable_params, IN const theoretical_static_param *static_params) {
	double F = variable_params->get_forward();
	double K = variable_params->get_strike();
...
int fb_baw_theoretical_algorithm::Exception(IN double F, IN double K, IN double T, IN double r, IN double sig){
	if (!math_helper::greater(F, 0) || !math_helper::greater(K, 0) || !math_helper::greater(T, 0) || !math_helper::greater(sig, 0) || !math_helper::greater(r, 0) ||
...
void fb_baw_theoretical_algorithm::on_timer_uncommon_greeks() {
	...
	for (auto iter = updated_uncommon_greeks_map_.begin(); iter != updated_uncommon_greeks_map_.end(); iter++) {
		...
		uncommon_greeks_baw(input.option_type, input.F, input.K, input.T, input.r, input.sig, &theta, &rho, &charm, &vanna, &vomma, &speed, &zomma);
...
int fb_baw_theoretical_algorithm::pricing_baw(IN unsigned short option_type, IN double F, IN double K, IN double T, IN double r, IN double sig, OUT double* theo_price, OUT double* delta, OUT double* gamma, OUT double* vega) {
...
int fb_baw_theoretical_algorithm::uncommon_greeks_baw(IN unsigned short option_type, IN double F, IN double K, IN double T, IN double r, IN double sig, OUT double* theta, OUT double* rho, OUT double* charm, OUT double* vanna, OUT double* vomma, OUT double* speed, OUT double* zomma) {
...
int fb_baw_theoretical_algorithm::single_baw(IN unsigned short option_type, IN double F, IN double K, IN double T, IN double r, IN double sig, OUT double* theo_price) {
	if (theo_price == NULL) {
		return -1;
	}
...
double fb_baw_theoretical_algorithm::baw_call(IN unsigned short option_type, IN double F, IN double K, IN double T, IN double r, IN double sig) {
	...
	while (iterate) {
		...
		b = discount * math_helper::normal_cdf(d1) * (1.0 - gamma_2_inv) + gamma_2_inv * (1.0 - discount * math_helper::normal_pdf(d1) / sig / std::sqrt(T));
		//calculate the Black Scholes value of a European call
		black_scholes_merton(option_type, S_star, K, T, r, sig, &eur_val_iter);
		rhs = eur_val_iter + (1.0 - std::exp(-r * T) * math_helper::normal_cdf(d1)) * S_star * gamma_2_inv;
		...
		if (flag > MAX_ITER) {
			PLOG("fb_baw_theoretical_algorithm::%s, the calculation excessed the maximum iteration limit %lf.\n", __FUNCTION__, MAX_ITER);
			return -1;
		}
	}
...
double fb_baw_theoretical_algorithm::baw_put(IN unsigned short option_type, IN double F, IN double K, IN double T, IN double r, IN double sig) {
	double alpha = 2.0 * r / sig / sig;
	double h = 1.0 - std::exp(-r * T);
	double d1, b, eur_val_iter, rhs, ERR, eur_option_val;
	double discount = std::exp(-r * T);
	...
	while (iterate) {
		...
		b = discount * (math_helper::normal_cdf(d1) * (1.0 - gamma_1_inv) - 1.0) + gamma_1_inv * (discount - 1.0 - ((discount * math_helper::normal_pdf(d1)) / sig / std::sqrt(T)));
		//calculate the Black Scholes value of a European put
		black_scholes_merton(option_type, S_star, K, T, r, sig, &eur_val_iter);
		rhs = eur_val_iter - (1.0 - std::exp(-r * T) + std::exp(-r * T) * math_helper::normal_cdf(d1)) * S_star * gamma_1_inv;
		S_star = (K - rhs + b * S_star) / (1.0 + b);
...
int fb_baw_theoretical_algorithm::black_scholes_merton(IN unsigned short option_type, IN double F, IN double K, IN double T, IN double r, IN double sig, OUT double* eur_val) {
	...
	if (option_type == PLUGIN_OPTION_CALL){
		*eur_val = discount_factor * (F * math_helper::normal_cdf(d1) - K * math_helper::normal_cdf(d2));
	}
	else if (option_type == PLUGIN_OPTION_PUT){
		*eur_val = discount_factor * (-F * math_helper::normal_cdf(-d1) + K * math_helper::normal_cdf(-d2));
	}
...
int fb_baw_theoretical_algorithm::calc_delta_gamma(IN unsigned short option_type, IN double F, IN double K, IN double T, IN double r, IN double sig, IN double cur_price, OUT double* delta, OUT double* gamma) {
...
double fb_baw_theoretical_algorithm::calc_vega(IN unsigned short option_type, IN double F, IN double K, IN double T, IN double r, IN double sig) {
...
double fb_baw_theoretical_algorithm::calc_theta(IN unsigned short option_type, IN double F, IN double K, IN double T, IN double r, IN double sig, IN double cur_price) {
...
double fb_baw_theoretical_algorithm::calc_rho(IN unsigned short option_type, IN double F, IN double K, IN double T, IN double r, IN double sig, IN double cur_price) {
...
int fb_baw_theoretical_algorithm::calc_high_order_greeks(IN unsigned short option_type, IN double F, IN double K, IN double T, IN double r, IN double sig, OUT double* charm, OUT double* vanna, OUT double* vomma, OUT double* speed, OUT double* zomma) {
...
double fb_baw_theoretical_algorithm::payoff(IN double spot, IN double strike, IN unsigned short option_type) {
	if (option_type == PLUGIN_OPTION_CALL) {
...
double fb_baw_theoretical_algorithm::special_delta(IN double spot, IN double strike, IN unsigned short option_type) {
	if (spot > strike) {

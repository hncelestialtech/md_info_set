...
#include "fb_black76_theoretical_algorithm.h"
...
extern "C"
{
	...
	void* create() {
		return new cffex::fb::plugin::fb_black76_theoretical_algorithm();
	}
	void destroy(void* p) {
		delete (cffex::fb::plugin::fb_black76_theoretical_algorithm*)p;
	}
	void identity(void* handle, register_func f) {
		f(handle, 1, "fb_black76_theoretical_algorithm");
	}
...
fb_black76_theoretical_algorithm::fb_black76_theoretical_algorithm() {
}

fb_black76_theoretical_algorithm::~fb_black76_theoretical_algorithm() {
}

void fb_black76_theoretical_algorithm::set_register_timer_function(register_timer_callback f) {
	f(TIMER_INTERVAL, std::bind(&fb_black76_theoretical_algorithm::on_timer_uncommon_greeks, this), true);
};


int fb_black76_theoretical_algorithm::calculate(IN unsigned short option_type, IN const theoretical_variable_param* variable_params, IN const theoretical_static_param* static_params, IN const theoretical_custom_param *custom_params, IN const theoretical_volatility_param *volatility_params, OUT theoretical_field* f){
	...
	if (Exception(F, K, T, r, sig) == -1){
		PLOG("fb_black76_theoretical_algorithm::%s,there are exceptional data:F[%lf] or K[%lf] or T[%lf] or r[%lf] or sig[%lf] is invalid, failed to calculate\n", __FUNCTION__, F, K, T, r, sig);
		return -1;
	}
	T = std::max(T, T_MIN);
	...
	if (iter != base_map_.end()) {
		...
		if (math_helper::equal(input.r, r) && math_helper::greater_equal(input.T, T) && math_helper::less(input.T-T, MATURITY_TOLERANCE) && std::fabs(input.F-F)/input.F < UNDERLYING_TOLERANCE && std::fabs(input.sig-sig) < VOLATILITY_TOLERANCE) {
			...
			input_params input_updated(option_type, F, K, T, r, sig, theo_price);
	...
	calc_uncommon_greeks(option_type, F, K, T, r, sig, theo_price, &theta, &rho, &charm, &vanna, &vomma, &speed, &zomma);
	f->set_theta(theta);
	f->set_rho(rho);
	f->set_charm(charm);
	f->set_vanna(vanna);
	...
	full_params full(option_type, F, K, T, r, sig, theo_price, theta, rho, charm, vanna, vomma, speed, zomma);
...
int fb_black76_theoretical_algorithm::calculate(IN unsigned short option_type, IN const theoretical_variable_param* variable_params, IN const theoretical_static_param* static_params, IN const theoretical_custom_param *custom_params, IN const theoretical_volatility_param *volatility_params, OUT double* price){
	double F = variable_params->get_forward();
	double K = variable_params->get_strike();
	double T = variable_params->get_left_trading_years();
	double r = variable_params->get_rate();
...
int fb_black76_theoretical_algorithm::iv_derivable(IN unsigned short option_type, IN double option_price, IN const theoretical_variable_param* variable_params, IN const theoretical_static_param *static_params) {
	double F = variable_params->get_forward();
	double K = variable_params->get_strike();
...
/******** private ********/
int fb_black76_theoretical_algorithm::Exception(IN double F, IN double K, IN double T, IN double r, IN double sig){
...
void fb_black76_theoretical_algorithm::on_timer_uncommon_greeks() {
	...
	for (auto iter = updated_uncommon_greeks_map_.begin(); iter != updated_uncommon_greeks_map_.end(); iter++) {
		...
		double theta, rho, charm, vanna, vomma, speed, zomma;
		calc_uncommon_greeks(input.option_type, input.F, input.K, input.T, input.r, input.sig, input.cur_price, &theta, &rho, &charm, &vanna, &vomma, &speed, &zomma);
		full_params full(input.option_type, input.F, input.K, input.T, input.r, input.sig, input.cur_price, theta, rho, charm, vanna, vomma, speed, zomma);
		base_map_[instrument] = full;
	}
	updated_uncommon_greeks_map_.clear();
}

double fb_black76_theoretical_algorithm::black76(IN unsigned short option_type, IN double F, IN double K, IN double T, IN double r, IN double sig) {
...
double fb_black76_theoretical_algorithm::calc_delta(IN unsigned short option_type, IN double F, IN double K, IN double T, IN double r, IN double sig) {
	double d1 = std::log(F / K) + 0.5 * sig * sig * T;
	d1 = d1 / sig / std::sqrt(T);
	if (option_type == PLUGIN_OPTION_CALL) {
...
double fb_black76_theoretical_algorithm::calc_gamma(IN double F, IN double K, IN double T, IN double r, IN double sig) {
	double d1 = std::log(F / K) + 0.5 * sig * sig * T;
	d1 = d1 / sig / std::sqrt(T);
	return math_helper::normal_pdf(d1) * std::exp(-r * T) / F / sig / std::sqrt(T);
}

double fb_black76_theoretical_algorithm::calc_vega(IN double F, IN double K, IN double T, IN double r, IN double sig) {
	double d1 = std::log(F / K) + 0.5 * sig * sig * T;
	d1 = d1 / sig / std::sqrt(T);
	return F * std::exp(-r * T) * math_helper::normal_pdf(d1) * sqrt(T) / 100;
}

int fb_black76_theoretical_algorithm::calc_uncommon_greeks(IN unsigned short option_type, IN double F, IN double K, IN double T, IN double r, IN double sig, IN double cur_price, OUT double *theta, OUT double *rho, OUT double *charm, OUT double *vanna, OUT double *vomma, OUT double *speed, OUT double *zomma) {
	...
	if (option_type == PLUGIN_OPTION_CALL) {
		if(math_helper::less_equal(tomorrow_T, T_MIN)) {
			*theta = std::max(F - K, 0.0) - cur_price;
		} else {
		...
		*charm = -discount_factor * (-d2 * d1_npdf / 2 / T - r * d1_ncdf) / TRADING_DAYS;
		...
		if(math_helper::less_equal(tomorrow_T, T_MIN)) {
			*theta = std::max(K - F, 0.0) - cur_price;
		} else {

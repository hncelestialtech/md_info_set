...
class fb_black76_theoretical_algorithm : public i_theoretical_algorithm {
public:
	class input_params {
	public:
		input_params() {}
		input_params(unsigned short option_type, double F, double K, double T, double r, double sig, double cur_price)
			:option_type(option_type), F(F), K(K), T(T), r(r), sig(sig), cur_price(cur_price){};
	public:
		unsigned short option_type;
		double F, K, T, r, sig, cur_price;
	};
	...
	public:
		full_params() {}
		full_params(unsigned short option_type, double F, double K, double T, double r, double sig, double cur_price, double theta, double rho, double charm, double vanna, double vomma, double speed, double zomma)
			:input_(option_type, F, K, T, r, sig, cur_price), uncommon_greeks_(theta, rho, charm, vanna, vomma, speed, zomma) {};
	public:
		input_params input_;
		uncommon_greeks_params uncommon_greeks_;
	};

	fb_black76_theoretical_algorithm();
	virtual ~fb_black76_theoretical_algorithm();
...
private:
	double black76(IN unsigned short option_type, IN double F, IN double K, IN double T, IN double r, IN double sig);
	double calc_delta(IN unsigned short option_type, IN double F, IN double K, IN double T, IN double r, IN double sig);
	double calc_gamma(IN double F, IN double K, IN double T, IN double r, IN double sig);
	double calc_vega(IN double F, IN double K, IN double T, IN double r, IN double sig);
	int calc_uncommon_greeks(IN unsigned short option_type, IN double F, IN double K, IN double T, IN double r, IN double sig, IN double cur_price, OUT double* theta, OUT double* rho, OUT double* charm, OUT double* vonna, OUT double* vomma, OUT double* speed, OUT double* zomma);
private:

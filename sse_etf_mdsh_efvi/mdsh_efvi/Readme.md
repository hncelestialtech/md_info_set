...
## 功能概述
---
本工具用于接收上交所mdsh行情，适配solarflare网卡(如需其他网卡类型，请联系对接人员)，并支持用户自定义函数回调对数据进行处理。
...
---
### 软件部署
1、在预设目录下解压代码包,以本地版为例，压缩包文件名：mdsh_efvi.tar
```
tar -xvf mdsh_efvi.tar
```
内含文件：
> include  
>> MdshReceiveEfvi.h  
...
nic_type=efvi                                                   //网卡类型 此版本固定为exanic
[license_file]
...
### 3. v1.2.0_20221210
接口命名规范性调整，增加期权模式、订阅模式，增加配置文件日志路径

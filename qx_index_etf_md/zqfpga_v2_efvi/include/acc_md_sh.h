...
#define MD_TYPE_SH_L1 0x00010001U        // 上交所L1快照 0x00010001
...
typedef struct t_SH_MCBMarketDataL1
{
	int    msg_type;																	//消息类型 
    int    msg_len;                                  //消息长度 
    char security_id[8];                    //产品代码
    unsigned long long send_time;           //行情时间，格式：YYYYMMDDHHmmSSsss
    unsigned long long pre_close_price;     //昨收盘价,除以100000
    unsigned long long open_price;          //开盘价,除以100000
    unsigned long long high_price;          //最高价,除以100000
    unsigned long long low_price;           //最低价,除以100000
    unsigned long long match_price;         //最新价,除以100000
    unsigned long long iopv;                //基金IOPV,除以100000
    unsigned long long pre_iopv;            //基金前一日IOPV,除以100000
    unsigned long long bid_price[5];        //买盘价格,除以100000
    unsigned long long bid_qty[5];          //买盘数量
    unsigned long long offer_price[5];      //卖盘价格,除以100000
    unsigned long long offer_qty[5];        //卖盘数量
    unsigned long long total_volume_traded; //成交数量
    unsigned long long num_trades;          //成交笔数
    unsigned long long total_value_traded;  //成交金额,除以100
    char sTradingPhaseCode[8];              //实时阶段及标志
} T_SH_MCBMarketDataL1, *PSH_MCBMarketDataL1;
#pragma pack(pop)
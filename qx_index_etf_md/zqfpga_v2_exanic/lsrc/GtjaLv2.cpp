...
void RcvLoopExanic()
{
    ...
    while (0 == g_StopProcess)
    {
        ...
        auto expand_timestamp = exanic_expand_timestamp(g_exanic, timestamp);
        if (sz > 0)
        {
...
void *ThreadProc(void *arg)
{
    ...
    ofstream f_shse_snapshot, f_shse_index, f_shse_orderq_bid, f_shse_orderq_ask, f_shse_option, f_shse_bond_l2, f_shse_bond_zb, f_shse_merge_tick;
    ...
    while (0 == g_StopProcess)
    {
        if ((int32_t)(g_WriteIdx - g_ReadIdx) > 0)
        {
            ...
            auto ts = pItem->TimeStamp;
            auto timestamp = exanic_cycles_to_ns(g_exanic, static_cast<exanic_cycles_t>(ts));
            int msgtype = 0;
